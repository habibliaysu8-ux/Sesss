...
APP_VERSION = 'v4-photo-responsive-20260817'
...
<section id="create" class="screen">
  ...
  <div class="content">
    <div class="panel">
      ...
      <label for="postImage" class="photo-button">📷 Fotoğraf ekle <span style="font-weight:500;color:#7a8498">(isteğe bağlı)</span></label>
...
<section id="profile" class="screen">
  ...
  <div class="content">
    ...
    <label for="profileImage" class="photo-button" style="margin-top:8px">📷 Profil fotoğrafı ekle / değiştir</label>
...
async function createPost(){
  ...
  const f=document.getElementById('postImage').files[0];if(f)fd.append('image',f);
...
document.getElementById('profileImage').addEventListener('change',async e=>{
  ...
  if(!f.type.startsWith('image/')){alert('Lütfen bir fotoğraf seç.');e.target.value='';return;}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path=="/api/posts":
            ...
            c.commit();c.close();return self.send_json({"id":cur.lastrowid},201)
        m=re.fullmatch(r"/api/posts/(\d+)/comments",path)
        if m:
